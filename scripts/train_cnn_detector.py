...
def try_train(config):
    ...
    pool_sizes = [0] * (config['n_conv_layer0'] - 1) + [10] \
                 + [0] * (config['n_conv_layer1'] - 1) + [10]
    pool_strides = [None] * (config['n_conv_layer0'] - 1) + [10] \
                 + [None] * (config['n_conv_layer1'] - 1) + [10]
...
def main():
    ...
    result = tune.run(
        try_train,
        name='CNNDetector' + '_' + datetime.now().strftime('%Y-%m-%d_%H-%M-%S'),
